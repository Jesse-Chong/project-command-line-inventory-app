...
function listItems() {
  ...
  if (data.length === 0) {
    ...
    console.log(chalk.redBright("No items found."));
  } else {
    // If data array is not empty than the code moves on
    console.log(chalk.cyanBright.bold("List of items:")); // Heading
...
function updateItem(itemId, updatedItem) {
  ...
  if (itemIndex !== -1) {
    ...
    writeJSONFile(data);
    console.log(chalk.greenBright("Item updated successfully."));
  } else {
...
function removeItem(itemId) {
  ...
  if (dataIndex === -1) {
    ...
    writeJSONFile(data);
    console.log(chalk.redBright("Item removed successfully"));
  }
}
...
function viewItem(itemId) {
  ...
  if (item) {
    console.log(chalk.cyanBright.bold("==== Item Details ===="));
    console.log(chalk.cyan(` Name: ${item.name}`));
    console.log(chalk.cyan(` Price: $${item.price}`));
    console.log(chalk.cyan(` Color: ${item.color}`));
    console.log(
